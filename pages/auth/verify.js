      ...
      <Card className="shadow p-4 rounded">
        {reqStatus === "success" ? (
          <div className="d-flex" style={{ flexWrap: "wrap" }}>
            ...
            <h1>Email Sent</h1>
            <h4 className="mt-3">
              Thanks! Check your inbox for {session?.user?.email} you'll get an
              email with a link to verify your account shortly. If it is missing
              please check your spam folder as well.
            </h4>
          </div>
        ) : (
          <h4 className="my-4 mx-auto">
            To verify your account we will send an email to{" "}
            <strong>{session?.user?.email}</strong>. To begin, please click on
            the button below
          </h4>
        )}
        <Row>
          ...
          {!reqStatus && (
            <Button className="mx-auto my-4" onClick={handleClick}>
              Send Email
            </Button>
          )}
        </Row>
...
export async function getServerSideProps(context) {
  ...
  if (!token)
    return {
      props: {
        resend: true,
        msg: "Your verification link may have expired. Please click on resend to send another verification email.",
      },
    }
  ...
  if (!user)
    return {
      props: {
        resend: true,
        msg: "The account associated with this link could not be located",
      },
    }
  if (user.verified)
    return { props: { msg: "Your email has already been verified" } }
