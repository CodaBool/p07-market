...
export default applyMiddleware(async (req, res) => {
  try {
    ...
    const env = new sdk.core.SandboxEnvironment(
      process.env.NEXT_PUBLIC_PAYPAL_ID,
      process.env.PAYPAL_SK,
    )
    ...
    if (method === "POST") {
      if (body.orderID) {
        ...
        const charges = result.purchase_units[0].payments.captures.map(
          charge => ({
            _id: charge.id,
            pay_status: charge.status,
            amount: convertPayPalAmount(charge.amount.value),
            fee: convertPayPalAmount(
              charge.seller_receivable_breakdown.paypal_fee.value,
            ),
        ...
        const orderData = {
          ...
          amount_received: convertPayPalAmount(
          ...
          shipping: {
            ...
            address: {
              ...
              postal_code:
                result.purchase_units[0].shipping.address.postal_code,
        ...
        const { vendorLines, total, orderLines } = createOrderValidation(
          products,
          body,
          "paypal",
        )
...
async function storeOrder(body, paypal, headers) {
  ...
  console.log(
    ...
    body.details.purchase_units[0].payee.merchant_id,
  )
  console.log(
    "checking",
    process.env.PAYPAL_SANDBOX_SELLER_ACCOUNT,
    body.details.purchase_units[0].payee.email_address,
  )
  if (
    process.env.PAYPAL_MERCHANT_ID !==
    body.details.purchase_units[0].payee.merchant_id
  ...
  let valid = {
    error: `No such PayPal order "${body.details.id}"`,
    wh_verified: false,
  }
  if (paypalOrder) {
    ...
    valid = {
      wh_verified: false,
      id:
        paypalOrder.id === body.details.id
          ? true
          : `${paypalOrder.id}!=${body.details.id}`,
      status:
        paypalOrder.status === "COMPLETE"
          ? true
          : `${paypalOrder.status}!=COMPLETE`,
      local_amount:
      ...
      req_amount:
        ...
        body.details.purchase_units[0].payments.captures[0].amount.value
          ? true
      ...
      currency:
        paypalOrder.purchase_units[0].payments.captures[0].amount
          .currency_code ===
        body.details.purchase_units[0].payments.captures[0].amount.currency_code
          ? true
      ...
      status:
        paypalOrder.purchase_units[0].payments.captures[0].status ===
        body.details.purchase_units[0].payments.captures[0].status
          ? true
